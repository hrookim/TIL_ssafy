# 02_Props_Emit_Router

## SFC(Single File Component)

### Component

* 기본 HTML 엘리먼트를 확장하여 새자송 가능한 코드를 캡슐화하는데 도움을 준다
...
* 화면의 특정 영영에 대한 HTML, CSS, JavaScript 코드를 하나의 파일(`.vue`)에서 관ㄹ
...
> * HTML, CSS 그리고 SJ를 .vue라는 확장자를 가진 파일 안에서 관리하며 개발
...
* 즉, 단순히 브라우저만 조작할 수 있던 JS는 SSR 아키텍처에서도 사용할 수 있도록 한다.
...
* 괒거 자바스크립트의 파편화와 표준화의 영향으로 코드의 스펙트럼이 매우 다양했다
...
* Module 의존성 문제
  * 모듈의 수가 많아지고 라이브버리 혹은 모듈 간의 의존성(연결성)이 깊어지면서 특정한 곳에서 발생한 문제가 어떤 모듈 간의 문제인지 파악하기 어려움
...
> 2. Babel
>    * Compiler
>    * ES2016+ JS코드를 구 버전의 JS로 바꿔주는 도구
> 3. Webpack
>    * Module Bundler
>    * 모듈 간의 의존성 문제를 해결하기 위한 도구
...
* 컴포넌트간 부조-자식 관계가 구성되며 이들 사이에 필연적으로 <u>의사소통이 필요하다</u>
...
* props를 부모(상위) 컴포넌트의 정보를 전달하기 위한 사용자 지정 특성
...
* 수신할 prop 데이터를 명시적으로 선언한 후, 하위 컴포넌트에서 사용한다.
...
* 모든 props를 하위 속성과 상위 속성 사이의 단뱡향 바인딩을 형성한다
